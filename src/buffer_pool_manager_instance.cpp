...
BufferPoolManagerInstance::BufferPoolManagerInstance(size_t pool_size, uint32_t num_instances, uint32_t instance_index, DiskManager *disk_manager, LogManager *log_manager)
  ...
  // Asignamos un espacio de memoria consecutivo para la reserva de búferes.
  pages_ = new Page[pool_size_];
  replacer_ = new LRUReplacer(pool_size);

  // Inicialmente, todas las páginas están en la lista libre.
...
bool BufferPoolManagerInstance::FlushPgImp(page_id_t page_id) {
  // ¡Asegúrese de llamar a DiskManager::WritePage!
  return false;
}
...
Page *BufferPoolManagerInstance::NewPgImp(page_id_t *page_id) {
  // 0. ¡Asegúrate de llamar a AllocatePage!
  // 1. Si todas las páginas de la reserva de búferes están fijadas, devuelve nullptr.
  // 2. Escoge una página víctima P de la lista libre o del sustituto. Siempre se elige primero de la lista libre.
  // 3. Actualizar los metadatos de P, poner a cero la memoria y añadir P a la tabla de páginas.
